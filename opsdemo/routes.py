...
def invoice_send_email(invoice_id: int):
    ...
    smtp_cfg = dict(get_smtp_config())
    if not _email_configured(smtp_cfg):
        flash("Email is not configured. Go to Settings → Email.", "warning")
        return redirect(url_for("main.invoices"))
    # Inject org display name if configured
    if g.user and g.user.org and g.user.org.email_from_name:
...
def _build_table_pdf(title: str, headers: list, rows: list,
    ...
    try:
        if g.user and g.user.org:
            _hdr_text = g.user.org.pdf_header_text or g.user.org.name or app_title
            _ftr_text = g.user.org.pdf_footer_text
    except Exception:
        pass
    footer_line = _ftr_text or f"Generated by {app_title}"
    ...
    # Header bar
    hdr = Table([[
        _p(f"<b>{_hdr_text}</b>", size=13, color=WHITE),
    ...
    elements.append(Paragraph(footer_line, foot_s))
...
@bp.route("/settings/brand", methods=["GET", "POST"])
@admin_required
def settings_brand():
    org = db.session.get(Organization, g.user.org_id) if g.user.org_id else None
    if not org:
    ...
    if request.method == "POST":
        ...
        if logo_file and logo_file.filename:
            ...
            if ext not in _ALLOWED_LOGO_EXTENSIONS:
                flash("Unsupported file type. Use PNG, JPG, SVG, GIF, or WebP.", "danger")
                return redirect(url_for("main.settings_brand"))
            filename = secure_filename(f"org_{org.slug}.{ext}")
            logo_file.save(os.path.join(_logo_upload_dir(), filename))
            org.logo_path = f"uploads/logos/{filename}"
            log_audit("brand_logo_upload", "settings", related_record=org.name,
                      message=f"Logo updated for org '{org.name}'")

        # Remove logo
        if request.form.get("remove_logo") == "1" and org.logo_path:
            try:
                full_path = os.path.join(current_app.static_folder, org.logo_path)
        ...
        org.pdf_header_text = request.form.get("pdf_header_text", "").strip() or None
        org.pdf_footer_text = request.form.get("pdf_footer_text", "").strip() or None
        org.email_from_name = request.form.get("email_from_name", "").strip() or None

        db.session.commit()
        flash("Brand settings saved.", "success")
        return redirect(url_for("main.settings_brand"))

    return render_template("settings_brand.html", org=org)
